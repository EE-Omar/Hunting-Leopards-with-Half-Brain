...
|-------|------|-------------|-----------|---------|
| **1** | `create_split_model.py` | Splits best_640.onnx into backbone + head | Pi | `python3 create_split_model.py models/best_640.onnx models/split_640 640` |
| **2** | `verify_split_new.py` | Checks if split works correctly (100% match?) | Pi| `python3 verify_split_new.py models/best_640.onnx models/split_640/backbone_640.onnx models/split_640/head_640.onnx images/ 640` |
...
| **4a** | `laptop_head_client.py` | Starts head server (waits for Pi) | Laptop | `python3 scripts/laptop_head_client.py models/split_640/head_640.onnx --port 5000 --imgsz 640` |
| **4b** | `pi_backbone_server.py` | Runs backbone, sends tensors to laptop | Pi | `python3 scripts/pi_backbone_server.py models/split_640/backbone_640.onnx <LAPTOP_IP> 5000 640 --images images/` |
...
```bash
# Step 1: Split (run once on Pi with models/)
cd ~/Second_Try
