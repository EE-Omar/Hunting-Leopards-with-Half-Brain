"""
Cache layer-3 activations so the bottleneck can be trained without the frozen YOLO in
the loop. This is what makes CPU-only training practical: the ~70K-param autoencoder
...
def main():
    parser = argparse.ArgumentParser(description="Cache YOLO26 layer-3 features to disk")
    ...
    parser.add_argument("--out", default=str(ROOT / "models/bottleneck/feats_320.npy"))
    parser.add_argument("--imgsz", type=int, default=320)
    parser.add_argument("--limit", type=int, default=3000,
                        help="Number of images to cache (3000 gives ~4.8M feature vectors)")
    ...
    print(f"[*] Backbone stub (layers 0-3): {n_params:,} params")

    images = list_images(args.images, args.limit, args.seed)
    grid = args.imgsz // 8  # layers 0-3 downsample by 8
    ...
    print(f"[*] Layer-3 feature distribution (n={sample.size:,}):")
