...
Three-way accuracy comparison of the ACTUAL DEPLOYED ONNX ARTIFACTS.
    ...
    full        models/full/best_320.onnx
    split       backbone_320.onnx -> head_320.onnx
    bottleneck  backbone_enc_320.onnx -> uint8 round-trip -> dec_head_320.onnx

Unlike eval_bottleneck.py (which evaluates the PyTorch model with the bottleneck wrapped
into net.model[3]), this runs the exact ONNX files that ship to the Pis, including the
...
def main():
    ...
    ap_.add_argument("--out", default=str(ROOT / "results/benchmarks/accuracy_three_way_v3.csv"))
    ap_.add_argument("--imgsz", type=int, default=320)
