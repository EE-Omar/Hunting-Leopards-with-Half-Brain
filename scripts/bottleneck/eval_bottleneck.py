...
The v3 model was trained at 640, so its mAP at 320 is not a known quantity. Measure the
unmodified 320 baseline FIRST — it is the only fair comparison target for the bottleneck.

Usage:
    # Baseline: unmodified model at 320 (run this before anything else)
    python scripts/bottleneck/eval_bottleneck.py --baseline-only --imgsz 320

    # With a trained bottleneck injected at layer 3
    python scripts/bottleneck/eval_bottleneck.py --ckpt models/bottleneck/bneck_16ch.pt --imgsz 320
...
from scripts.bottleneck.modules import SPLIT_CHANNELS, SPLIT_LAYER, load_bottleneck, wrap_split_layer  # noqa: E402


def evaluate(weights, data, imgsz, batch, device, split, ckpt=None, quantize=True):
    """Run Ultralytics validation, optionally with the bottleneck wrapped in at the split layer."""
    ...
    if ckpt is not None:
        ...
        wrap_split_layer(model.model, block)
        scale, zero_point = block.quant.qparams()
        print(f"[*] Bottleneck injected at layer {SPLIT_LAYER}")
        ...
        raw_kb = SPLIT_CHANNELS * (imgsz // 8) ** 2 * 4 / 1024
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate the layer-3 bottleneck's effect on mAP")
    parser.add_argument("--weights", default=str(ROOT / "models/full/best_v3.pt"),
                        help="PyTorch weights (default: models/full/best_v3.pt)")
