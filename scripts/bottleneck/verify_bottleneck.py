...
Compares, per image:
    full model                        best_256.onnx
    bottleneck path   backbone_enc_256.onnx -> uint8 -> dequant -> dec_head_256.onnx
...
Usage:
    python scripts/bottleneck/verify_bottleneck.py data/images/ --imgsz 256
"""
...
def main():
    ...
    raw_bytes = 128 * (args.imgsz // 8) ** 2 * 4
    wire_bytes = None
    ...
    print("WIRE")
    print(f"{'=' * 66}")
    print(f"  fp32 layer-4 tensor : {raw_bytes / 1024:>8.1f} KB/frame")
