"""
Learned bottleneck inserted at the YOLO26-Large layer-3 split point.

The split tensor is (1, 256, 40, 40) at imgsz=320 — 1600 KB/frame as fp32, which is
what currently makes split inference lose to running the whole model on-device. This
module squeezes it to (1, 16, 40, 40) and quantizes to uint8 on the wire: 25 KB/frame.

Layer 3 is a plain stride-2 Conv feeding only layer 4, so it is a clean single cut —
unlike layer 4, which also feeds the P3 concat skip. Wrapping layer 3 (whose output is
the tensor Ultralytics saves) puts the bottleneck on the one path that crosses the wire.

Design constraints:
  - The encoder runs on the field device, so it must be nearly free. Layers 0-3 are only
    ~840K params but are FLOP-heavy because they run at high spatial resolution. A 1x1
    conv at the 40x40 grid costs ~6.6M MACs — a rounding error next to the backbone.
...
SPLIT_LAYER = 3
SPLIT_CHANNELS = 256
# The ONNX tensor name for that layer's output — the boundary both split halves cut on.
SPLIT_TENSOR = "/model.3/act/Mul_output_0"
...
class BottleneckBlock(nn.Module):
    """encode -> (fake) quantize -> decode, operating on the layer-3 feature map."""
    ...
    def wire_bytes(self, spatial=40):
        """Bytes actually placed on the wire per frame, at uint8."""
        return self.ch_bottleneck * spatial * spatial


class LayerWithBottleneck(nn.Module):
    """Wraps the original split-point module so the bottleneck rides on its output.
...
def wrap_split_layer(net, bottleneck):
    """Insert `bottleneck` after layer SPLIT_LAYER of a DetectionModel, in place.
    ...
    if isinstance(current, LayerWithBottleneck):
        current.bottleneck = bottleneck  # already wrapped — just swap the block
        return net
    net.model[SPLIT_LAYER] = LayerWithBottleneck(current, bottleneck)
    return net


def backbone_stub(net):
    """Layers 0-3 as a standalone Sequential, for feature extraction.
